...
namespace Medicina.Entidades
{
  [Serializable]
	...
	TimeSpan _horaHasta;
	TGEFiliales _filial;
        List<MedEspecializaciones> _especializacion;
    int _tiempo;
	...
	public TGEFiliales Filial
	{
      ...
      [Auditoria()]
    public List<MedEspecializaciones> Especializaciones
    {
        get { return _especializacion == null ? (_especializacion = new List<MedEspecializaciones>()) : _especializacion; }
        set { _especializacion = value; }
    }

        public string EspecializacionString
        {
            get
            {
                string union = string.Empty;
                string separador = string.Empty;
                foreach (MedEspecializaciones lista in Especializaciones)
                {
                    union = String.Concat(union, separador, lista.Descripcion);
                    separador = "<br>";
                }
                return union;
            }
