...
{
    public partial class EdificiosConsultar : PaginaSegura
    {
        protected override void PageLoadEvent(object sender, EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                if (!this.MisParametrosUrl.Contains("IdEdificio"))
                    this.Response.Redirect(AyudaProgramacion.ObtenerUrlParametros("~/Modulos/LavaYa/EdificiosListar.aspx"), true);

                int parametro = Convert.ToInt32(this.MisParametrosUrl["IdEdificio"]);
                edificio.IdEdificio = parametro;

                ModificarDatos.IniciarControl(edificio, Gestion.Consultar);
            }
        }
